...
def require_auth(func):
    ...
    async def wrapper(update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if not is_authorized(user_id):
            msg = "⛔ غير مصرح لك!" + "\n\n" + "🔒 ليس لديك صلاحية." + "\n" + "📩 تواصل مع المشرف." + "\n\n" + "🆔 معرفك: " + user_id
            await reply_to_user(update, msg)
            return
...
# ============ Helper for reply ============
async def reply_to_user(update: Update, text: str, parse_mode: str = "HTML"):
    """Reply to user whether from command or callback query"""
    if update.message:
        await reply_to_user(update, text, parse_mode=parse_mode)
...
parse_mode="HTML", reply_markup=reply
    if update.message:
        ...
        await update.callback_query.message.reply_text(text, parse_mode="HTML", reply_markup=reply)
...
⏱️ وقت التشغيل: {uptime_str}"""
    await reply_to_user(update, msg, parse_mode="HTML")
...
async def cmd_price(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        await reply_to_user(update, f"❌ خطأ: {e}")

@require_auth
async def cmd_signal(update: Update, context: ContextTypes.DEFAULT_TYPE):
    async with db_lock:
        if not db["signals"]:
            await reply_to_user(update, "⏳ لا توجد اشارات بعد")
            return
        last = db["signals"][-1]
    await reply_to_user(update, f"📈 <b>آخر اشارة</b>\n\n{last['text']}", parse_mode="HTML")
...
async def cmd_equity_chart(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await reply_to_user(update, "⏳ جاري انشاء الرسم...")
    ...
    if chart_path:
        ...
        await reply_to_user(update, "❌ فشل انشاء الرسم")
...
async def cmd_atr(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await reply_to_user(update, f"⚡ <b>ATR</b>\nالحالي: {atr:.2f}\nالحد: {threshold}\nالحالة: {status}", parse_mode="HTML")
...
async def cmd_errors(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not errors:
        await reply_to_user(update, "✅ لا اخطاء")
        return
...
async def cmd_force_analysis(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if is_weekend():
        ...
        return
    await reply_to_user(update, "🔄 <b>جاري التحليل الفوري...</b>", parse_mode="HTML")
    try:
        tf_data = await fetch_all_tf()
        dxy_price = await fetch_dxy_price()
        missing = [k for k, v in tf_data.items() if not v]
        if missing:
            await reply_to_user(update, f"⚠️ بيانات ناقصة: {', '.join(missing)}")
            return
        ...
        if signal is None:
            if consensus_status == "DISAGREEMENT":
                await reply_to_user(update, "❌ <b>خلاف بين النماذج</b>\nالقرار: HOLD\nانتظر فرصة أوضح", parse_mode="HTML")
            else:
                await reply_to_user(update, "❌ <b>خطأ في التحليل</b>\nحاول مرة أخرى", parse_mode="HTML")
            return
        ...
        await reply_to_user(update, f"❌ <b>خطأ:</b>\n{str(e)}")
    ...
    if context.args:
        try:
            ...
            if 0.1 <= new_risk <= 5.0:
                ...
                await save_state()
                await reply_to_user(update, f"✅ <b>تم التعديل:</b> <code>{new_risk}%</code>", parse_mode="HTML")
            else:
                await reply_to_user(update, "❌ بين 0.1% و 5.0%", parse_mode="HTML")
        except ValueError:
            await reply_to_user(update, "❌ استخدم: /risk 1.5", parse_mode="HTML")
    else:
        await reply_to_user(update, f"📊 المخاطرة: <code>{current_risk}%</code>", parse_mode="HTML")

@require_auth
async def cmd_pause(update: Update, context: ContextTypes.DEFAULT_TYPE):
    async with db_lock: db["paused"] = True
    await save_state()
    await reply_to_user(update, "⏸️ <b>تم الايقاف</b>", parse_mode="HTML")

@require_auth
async def cmd_resume(update: Update, context: ContextTypes.DEFAULT_TYPE):
    async with db_lock: db["paused"] = False
    await save_state()
    await reply_to_user(update, "▶️ <b>تم الاستئناف</b>", parse_mode="HTML")

@require_auth
async def cmd_news(update: Update, context: ContextTypes.DEFAULT_TYPE):
    async with db_lock: news_list = db["upcoming_news"][:5]
    if not news_list:
        await reply_to_user(update, "📰 لا توجد اخبار عاجلة قادمة")
        return
...
async def cmd_trades(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not trades:
        await reply_to_user(update, "📋 لا توجد صفقات مغلقة بعد")
        return
...
async def cmd_help(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await reply_to_user(update, "🤖 <b>الاوامر</b>\n"
"/start - القائمة\n"
"/status - الحالة\n"
"/price - الاسعار\n"
"/signal - آخر اشارة\n"
